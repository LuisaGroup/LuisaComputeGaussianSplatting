...
LUISA_STRUCT(lcgs::Camera, position, front, up, right, fov) {
...
public:
    explicit FPVCameraController(lcgs::Camera &camera,
                                 float move_speed,
                                 float rotate_speed,
...
int main(int argc, char *argv[]) {
    ...
    Kernel2D raytracing_kernel = [&](ImageFloat image, ImageUInt seed_image,
                                     Var<lcgs::Camera> camera, AccelVar accel, UInt2 resolution) noexcept {
    ...
    luisa::compute::Window window{"path tracing", resolution};
