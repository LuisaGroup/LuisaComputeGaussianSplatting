...
bool test_cam()
{
    ...
    // Test local_to_world and world_to_local matrixs are inverse of each other
    luisa::float4x4 l2w = local_to_world_matrix(camera);
    luisa::float4x4 w2l = world_to_local_matrix(camera);

    // Create a test point in local space
    luisa::float4 local_point = { 2.0f * M_SQRT2, 3.0f, 2.0f * M_SQRT2, 1.0f };

    // matrix to world space and back
    ...
    luisa::float4x4 proj = projection_matrix(tanfovx, tanfovy, near, far);
